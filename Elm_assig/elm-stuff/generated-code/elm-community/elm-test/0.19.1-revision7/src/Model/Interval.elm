module Model.Interval exposing (Interval, compare, full, length, oneYear, open, view, withDurationMonths,endString,startString, withDurationYears, getStart, getEnd, lengthString)

import Html exposing (Html, div, p, text)
import Html.Attributes exposing (class, style)
--import Model.Date as Date exposing (Date, Month, onlyYear)
...
compare (Interval intA) (Interval intB) =
    let
        orderByStart = Date.compare intA.start intB.start  
        orderByEnd = case (intA.end, intB.end) of
                        (Nothing, Nothing) -> EQ
                        (_, Nothing) -> GT
                        (Nothing, _) -> LT
    ...
    in
        orderByStart |> chainCompare orderByEnd

withDefault2 : a -> Maybe b -> (b -> a) -> a
withDefault2 def a f =
    case a of
        Just x -> f x
        Nothing -> def

getStart: Interval -> Date
getStart (Interval intr) = intr.start

getEnd: Interval -> Maybe Date
getEnd (Interval intr) = intr.end

startString: Date -> String
startString date = 
    let
        yearStr = "Year " ++ String.fromInt (Date.year date) ++ " "
        monthStr = "Month " ++ (case (Date.month date) of
                                    (Just a) -> Date.monthToString a
                                    Nothing -> " ")
    in
        yearStr ++ monthStr

endString: Maybe Date -> String
endString date = case date of
                    Nothing -> "Present"
                    Just a -> startString a

lengthString: (Int, Int) -> String
lengthString (a, b) = (String.fromInt a) ++ " " ++ (String.fromInt b)


displayLength: Interval -> Html msg
displayLength interval = 
    let
        intervLength = length interval
    in
        case intervLength of
           (Just a) -> p [class "interval-length"] [text (lengthString a)]
           Nothing -> p [class "interval-length"] [text " "]

view : Interval -> Html msg
view interval =
    let
        startInterval = getStart interval
        endInterval = getEnd interval
    in
        div [class "interval"] 
        [
            p [class "interval-start"] [text (startString startInterval)] ,
            p [class "interval-end"] [text (endString endInterval)],
            displayLength interval
        ]
