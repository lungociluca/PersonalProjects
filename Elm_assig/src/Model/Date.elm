...
monthsBetween dA dB =
    let
        month1 = withDefault -1 (month dA) monthToInt
        month2 = withDefault -1 (month dB) monthToInt
        returnNothing = (month1 == -1) || (month2 == -1)
    in
        if returnNothing then Nothing else Just <| abs(12*(year dA - year dB) + (month1 - month2))
...
compare (Date d1) (Date d2) =
    let
        ...
        compareYears = if yearDiff == 0 then EQ else (if yearDiff < 0 then LT else GT)
        month1 = withDefault 12 (d1.month) monthToInt
        month2 = withDefault 12 (d2.month) monthToInt
        monthDiff = month1 - month2
    in
        compareYears |> chainCompare (if monthDiff == 0 then EQ else (if monthDiff < 0 then LT else GT))
...
view (Date d) =
    let
        monthString = monthToString (withDefault Jan d.month (\x -> x))
        yearString = String.fromInt <| d.year
        toDisplay = "Month " ++ monthString ++ " Year " ++ yearString
    in
        div [] [text toDisplay]
